...
function computeStatoScadenza(scadeIl) {
  ...
  const ore = (new Date(scadeIl).getTime()-Date.now())/3_600_000;
...
function selectAlertCritici(kitchen) {
  ...
  tutti.forEach(g=>{
    ...
    else if(sc==="critica") alerts.push({id:g.id+"_sc1", tipo:"critica",      priorita:1, nome:g.name, location:g.location, valore:Math.round((new Date(g.expiresAt).getTime()-Date.now())/3_600_000)+"h"});
    else if(sc==="in_scadenza") alerts.push({id:g.id+"_sc2", tipo:"in_scadenza", priorita:2, nome:g.name, location:g.location, valore:Math.round((new Date(g.expiresAt).getTime()-Date.now())/3_600_000)+"h"});
...
function hoursUntil(iso) {
  if(!iso) return null;
  return (new Date(iso).getTime()-new Date().getTime())/3600000;
}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div>
        ...
        {showAddTavolo&&(<Card t={t} style={{padding:16,marginBottom:12}}><div style={{display:"flex",gap:10,alignItems:"center"}}><LuxInput value={nuovoTavolo.numero} onChange={e=>setNuovoTavolo(p=>({...p,numero:e.target.value}))} placeholder="N. tavolo" t={t} style={{flex:1}}/><LuxInput value={nuovoTavolo.coperti} onChange={e=>setNuovoTavolo(p=>({...p,coperti:e.target.value}))} type="number" placeholder="Coperti" t={t} style={{width:90}}/><Btn t={t} variant="gold" onClick={addTavolo} disabled={false}>Aggiungi</Btn></div></Card>)}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:10}}>
            {tavoli.map(tv=>{const sc=TAVOLO_C[tv.status]||TAVOLO_C.libero;return(
              <div key={tv.id} style={{borderRadius:14,border:`2px solid ${sc.border}`,background:sc.bg,padding:"14px 16px",cursor:"pointer",transition:"all 0.25s",position:"relative"}} onClick={()=>toggleTavoloStatus(tv.id)} onContextMenu={e=>{e.preventDefault();if(confirm(`Rimuovi tavolo ${tv.numero}?`))setTavoli(p=>p.filter(x=>x.id!==tv.id));}}>
                ...
                {tv.status==="occupato"&&tv.arrivatoIl&&<div className="mono" style={{fontSize:7,color:sc.tc,marginTop:2,opacity:0.6}}>{Math.floor((Date.now()-new Date(tv.arrivatoIl).getTime())/60000)}min</div>}
              </div>
            );})}
          </div>
        )}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {tab==="fifo"&&(<div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        {fifoAlert.sort((a,b)=>new Date(a.expiresAt).getTime()-new Date(b.expiresAt).getTime()).map(item=>{
          ...
          <Card t={t} style={{padding:20}}>
            ...
            <>
              ...
              <div style={{marginTop:12,padding:"10px 0",borderTop:`1px solid ${t.div}`,display:"flex",gap:12,flexWrap:"wrap"}}>
                {[{l:"≤25%",c:t.success,d:"Eccellente"},{l:"25–32%",c:t.gold,d:"Buono"},{l:"32–40%",c:t.warning,d:"Alto"},{l:"+40%",c:t.danger,d:"Critico"}].map(fc=>(
