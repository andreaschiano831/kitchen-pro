...
export function exportEconomatoCSV(kitchen: Kitchen) {
  ...
  const lines = [
    header.join(","),
    ...rows.map((r) => header.map((k) => escCSV((r as any)[k])).join(",")),
  ];

  dl(`economato-${kitchen.name}.csv`, lines.join("\n"), "text/csv;charset=utf-8");
}

export function exportKitchenDoc(kitchen: Kitchen) {
  const today = new Date().toISOString().slice(0, 10);

  const freezer = (kitchen.freezer || []).map((it) => ({
    name: it.name,
    qty: it.quantity,
    unit: it.unit,
    location: (it as any).location || "freezer",
  ...
  <html>
    <head>
      <meta charset="utf-8"/>
      <title>${kitchen.name} report</title>
      <style>
    ...
    <body>
      ...
      <table>
        <thead>
          <tr>
            <th>Prodotto</th><th>Qty</th><th>Unit</th><th>Loc</th><th>Scadenza</th><th>MIN (pz)</th>
          </tr>
        </thead>
        <tbody>
          ${freezer
            .map(
              (r) =>
                `<tr><td>${r.name}</td><td>${r.qty}</td><td>${r.unit}</td><td>${r.location}</td><td>${r.exp}</td><td>${r.min}</td></tr>`
            )
            .join("")}
        </tbody>
      </table>
      ...
      <table>
        <thead>
          <tr><th>Prodotto</th><th>Qty</th><th>Unit</th><th>Note</th><th>OK</th></tr>
        </thead>
        <tbody>
          ${economato
            .map(
              (x) =>
                `<tr><td>${x.name}</td><td>${x.quantity}</td><td>${x.unit}</td><td>${x.notes ?? ""}</td><td>${x.checked ? "✓" : ""}</td></tr>`
            )
            .join("")}
        </tbody>
      </table>
  ...
  // Word compat: HTML saved as .doc
  dl(`kitchen-report-${kitchen.name}-${today}.doc`, html, "application/msword");
}
