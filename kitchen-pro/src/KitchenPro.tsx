  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:t.bg,padding:24}}>
      <div style={{maxWidth:420,width:"100%"}}>
        ...
        <div style={{display:"flex",gap:6,padding:4,background:"#d4c9a8",borderRadius:12,marginBottom:20,border:"2px solid #b8a87a"}}>
          {[{k:"join",l:"Entra con codice"},{k:"create",l:"Crea cucina"}].map((m)=>(
            <button key={m.k} onClick={()=>setMode(m.k as any)} style={{flex:1,padding:"10px",borderRadius:9,border:"none",cursor:"pointer",
              background:mode===m.k?"#fff":"transparent",color:mode===m.k?"#1a1a1a":"#888",fontWeight:mode===m.k?600:400,
...
function BriefingPanel({ kitchen, t, onClose }) {
  ...
  async function generate() {
    ...
    const ctx = {
      cucina: kitchen?.name,
      data: new Date().toLocaleDateString("it-IT",{weekday:"long",day:"2-digit",month:"long"}),
      in_scadenza_72h: allItems.filter(x=>x.expiresAt&&new Date(x.expiresAt)>new Date()&&(new Date(x.expiresAt)-now)<72*3600000).slice(0,10).map(x=>({nome:x.name,ore:Math.round((new Date(x.expiresAt)-now)/3600000)})),
      scaduti: allItems.filter(x=>x.expiresAt&&new Date(x.expiresAt)<new Date()).slice(0,5).map(x=>x.name),
      sotto_par: allItems.filter(x=>x.parLevel>0&&x.quantity<x.parLevel).slice(0,8).map(x=>({nome:x.name,qty:x.quantity,par:x.parLevel,unit:x.unit})),
      preparazioni_da_fare: (kitchen?.preparazioni||[]).filter(p=>p.status!=="smistata").slice(0,10).map(p=>({nome:p.nome,qty:p.quantita,unit:p.unitaMisura})),
      brigata: kitchen?.members?.length||1,
    };
    try {
      const res = await callAI({systemPrompt:AI_PROMPTS.briefing.system,userContext:JSON.stringify(ctx),maxTokens:1000,expectJSON:true});
      setResult(res);
    } catch(e){setError("Errore: "+e.message);}
    setLoading(false);
  }
        ...
        {result&&(
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div style={{padding:"16px 20px",borderRadius:12,background:`linear-gradient(135deg,${t.secondary}22,${t.bgAlt})`,border:`1px solid ${t.secondary}44`}}>
              <div style={{fontFamily:"var(--serif)",fontSize:15,fontStyle:"italic",color:t.ink,lineHeight:1.5}}>{result.intestazione}</div>
            </div>
            {result.priorita_giornaliere?.length>0&&<div>
              <div className="mono" style={{fontSize:9,color:t.inkFaint,marginBottom:8}}>🎯 PRIORITÀ DEL GIORNO</div>
              {result.priorita_giornaliere.map((p,i)=>(
                <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"8px 12px",borderRadius:8,background:t.bgAlt,marginBottom:4}}>
                  <span className="mono" style={{color:t.gold,flexShrink:0,minWidth:18}}>{i+1}.</span>
                  <span style={{fontFamily:"var(--serif)",fontStyle:"italic",fontSize:13,color:t.ink}}>{p}</span>
                </div>
              ))}
            </div>}
            {result.mep_urgente?.length>0&&<div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {result.mep_urgente.map((m,i)=>(
                  <div key={i} style={{padding:"6px 12px",borderRadius:8,background:t.goldFaint,border:`1px solid ${t.goldDim}`}}>
                    <span style={{fontFamily:"var(--serif)",fontStyle:"italic",fontSize:12,color:t.ink}}>{m.nome}</span>
                    {m.da_fare_entro&&<span className="mono" style={{fontSize:9,color:t.gold,marginLeft:6}}>{m.da_fare_entro}</span>}
                  </div>
                ))}
              </div>
            </div>}
            {result.allerte_haccp?.length>0&&<div style={{padding:"12px 16px",borderRadius:10,background:t.accentGlow,border:`1px solid ${t.danger}44`}}>
              ...
              {result.allerte_haccp.map((a,i)=><div key={i} style={{fontFamily:"var(--serif)",fontStyle:"italic",fontSize:12,color:t.danger}}>• {a}</div>)}
            </div>}
            {result.nota_chef&&<div style={{padding:"14px 18px",borderRadius:10,background:t.bgAlt,borderLeft:`4px solid ${t.gold}`}}>
